...
class TznDb extends Tzn {
    ...
    function setPagination($pageSize,$page=1) {
   	...
   	function getIdKey(){return $this->_table."Id";}
 ...
 function pSort($link, $param, $field, $style='',
	...
	/* ----- Pagination ------ */
	 function pNavegationNumeric($link, $param  ) {
	 	//echo $this->_pageSize;
	 	$oHTML = '';
		if (intval($this ->_total / $this->_pageSize) > 0 && $this ->_total > $this ->_pageSize){
			$oHTML .= '<div class="form-actions" id="paginator-media" style="background-color:#FAFAFA;border-top: 1px solid #EEEEEE;margin-bottom:0px;" >
               <div class="pagination pagination-centered" style="margin-top:0px;margin-bottom:0px;" ><ul>';
			$oHTML .= $this->pPrevious($link, $param, 'Previous');
			$oHTML .= $this->pPagination($link, $param);
			$oHTML .= $this->pNext($link, $param, 'Next');
			$oHTML .= '</ul></div></div>';
			return $oHTML;
		}
		return false;
				...
				if ($this->_page == $i) {
					if (!empty($styleCurrent)) {
						if ($styleCurrent == '<b>') {
							...
							$oHTML .= '<li class="'.$styleCurrent.'" ><a>'.$i.'</a></li>';
						}
					} else {
						$oHTML .= $i;
					}
				} else {
					$oHTML .= $li1.$begin.$i.$end.$i.'</a><li>';
				}
			}
			return $oHTML;
		}
		...
		$styleOn = TZN_DB_PAGING_ENABLED, $styleOff = TZN_DB_PAGING_DISABLED)
	{
		...
		if (!$this->hasPrevious()) {
			if (!empty($styleOff)) {
				$oHTML .= '<li class="'.$styleOff.'"><a>'.$text.'</a></li>';
			} else {
				$oHTML .= $text;
			}
		} else {
			$begin = '<li><a href="javascript:'.$link.'(';
			/*if (preg_match('/\?/',$link)) {
				$begin .= '&';
			} else {
				$begin .= '?';
			...
			$end .= '>';
			$oHTML .= $begin.($this->_page-1).$end.$text.'</a></li>';
		}
		return $oHTML;
	}
