  ...
  function TznFile($type='image') {
	...
	function upload($field) {
		if (($_FILES[$field] == null) || ($_FILES[$field]['tmp_name'] == '')) {
			...
			$path_info = pathinfo($_FILES[$field]['name']);
		}
		// check file type
		...
		if ((!empty($type)) && (array_key_exists($type,$this->_mimeList))) {
			$this->fileExt = $this->_mimeList[$type];
		} elseif((!empty($path_info)) && (array_key_exists($path_info['extension'],$this->_mimeList))){
			$this->fileExt = $path_info['extension']; 
		}else{
			$this->_error[$field] = $GLOBALS['langTznDocument']['document_wrong_type'];
			echo $this->_error[$field];
			return false;
		}
