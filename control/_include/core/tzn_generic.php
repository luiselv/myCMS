...
class Tzn {	
	 ...
	 * 
	 * @param
	 *       	 prm1 array or name of property
	 * @param
	 *       	 prm2 if first parameter is, second is type (else: not needed)
	 */
	...
	function get($key) {
		...
		if ($dfn = $this->_properties [$key]) {
			...
			if ($arrType [0] == 'OBJ') {
				...
				;
				$arrArgs = func_get_args ();
				return call_user_func_array ( array (&$this, 'get' . $arrType [0] ), $arrArgs );
			} else {
				$type = $arrType [0];
				$arrType [0] = $key;
	...
	function gPrefix() {
		return (@constant('TZN_DB_PREFIX')?TZN_DB_PREFIX.'_':'');
	}
	function getUid() {
		return $this->id;
	}
	...
	function getDtm($keyval, $format = 'SQL', $tz = TZN_TZDEFAULT, $default = '') {
		...
		if ($value == 'now' || $value == 'NOW') {
			$value = TZN_NOW;
		} else if ((preg_match ( '/^(0000|9999)/', $value )) || (($dt = @strtotime ( $value )) === - 1)) {
	...
	function _value($keyval, $default = '') {
		if (is_object ( $this )) {
			if (preg_match ( "/^(.*)\((.*)\)$/i", $keyval, $m )) {
				$keyval = $m [1];
				if ($m [2]) {
					$value = call_user_func_array ( array (&$this, $keyval ), explode ( ',', $m [2] ) );
				} else {
					$value = $this->$keyval ();
				}
			} else {
	...
	function _dteValue($value, $format = 'SQL', $default = '') {
		...
		if (! $value || preg_match ( '/^(0000|9999)/', $value )) {
			...
			if ($tmp == - 1 || $tmp === false) {
				...
				// try to translate
				/*
				 * $value = str_replace( array_values($GLOBALS['langTznDate']),
				 * array_keys($GLOBALS['langTznDate']), $value);
				 */
				// error_log('unknown date value: '.$value.' /
				// '.$GLOBALS['langTznDate']['august']);
	...
	function dump($nested = '') {
		...
		foreach ( $this->_properties as $key => $type ) {
			if ($type == 'OBJ') {
				...
				echo $nested . $key . " [$type] => " . ((is_null ( $this->$key )) ? 'NULL' : $this->$key) . "\r\n";
			}
		}
	}
		...
		foreach ( $this->_properties as $key => $type ) {
			if ($type == 'LVL' && ($method == '_setFromHttp')) {
				...
				if ($nested) {
					...
					// $nid = $nested.'_'.$key.'Id';
					// echo "setting $key ID=".$data->$nid." to $key by $method
					// <br/>";
					$this->$key = $obj;
					unset ( $obj );
				} else {
				...
				/*
				 * if ($nested && ($method == '_setFromHttp')) { echo "$nested:
				 * setting $key as ".$this->$key." ($nKey) <br/>"; }
				 */
			}
		}
	}
	...
	function setHttp($key, $value) {
		if (! is_null ( $value )) {
			...
			/*
			 * $arrType = explode(',',$this->_properties[$key]); if ($key ==
			 * 'id') { $this->setUid($value); } else if ((!is_null($value)) ||
			 * ($arrType[0] == 'IMG')) { $arrArgs = func_get_args(); echo
			 * "calling... set".$arrType[0]." with <pre>"; print_r($arrArgs);
			 * echo "</pre>"; exit; }
			 */
		}
	}
	...
	function set($key, $value) {
		...
		if ($key == 'id') {
			...
			$arrArgs = func_get_args ();
			if ($arrType [0]) {
				return call_user_func_array ( array (&$this, 'set' . $arrType [0] ), $arrArgs );
			} else {
	...
	function setJSON($key, $value) {
		$this->$key = json_encode( $value,true );
	}	
	function setTxt($key, $value) {
		$this->$key = preg_replace ( "/<script[^>]*>[^<]+<\/script[^>]*>/is", "", $value );
	}
	...
	function setBbs($key, $value) {
		...
		$value = preg_replace ( "/<\/?(div|span|iframe|frame|input|" . "textarea|script|style|applet|object|embed|form)[^>]*>/is", "", $value );
		$this->$key = $value;
	}
	...
	function uplImg($key) {
		...
		$arrArgs = func_get_args();
		if (count ( $arrArgs ) == 1) {
			// if no specific parameter for size and folder, gets default from
			// properties
			$arrArgs = explode ( ',', $this->_properties [$key] );
			$arrArgs [0] = $key;
		}
		$ok = call_user_func_array ( array(&$objFile, 'upload' ), $arrArgs );
		// set value
		$tmpKey = $key . '_tmp';
		if ($ok) {
	...
	function p($key) {
		...
		if ($dfn = $this->_properties [$key]) {
			...
			if ($arrType [0] == 'OBJ') {
				...
				;
				return call_user_func_array ( array (&$this, 'p' . $arrType [0] ), $arrArgs );
			} else {
				$type = $arrType [0];
				$arrType [0] = $key;
	...
	function pEml($keyval, $default = '', $style = '') {
		...
		if ($value) {
			print '<a href="mailto:' . $value . '"' . Tzn::_style ( $style ) . '">' . $value . '</a>';
		} else {
			print $default;
		}
	}
	...
	function pBbs($keyval, $default = '') {
		$value = Tzn::_value ( $keyval, $default );
		$value = preg_replace ( "/(?<!\")((http|ftp)+(s)?" . ":\/\/[^<>\s]+)/i", "<a href=\"\\0\" target=\"_blank\">\\0</a>", $value );
		print nl2br ( $value );
	}
	...
	function qHidden($keyval, $default = '') {
		...
		$form = '<input type="hidden" id="' . $keyval . '" name="' . $keyval . '" value="' . $value . '" />';
		return $form;
	}
	function qText($keyval, $default = '',$style = '', $xtra = '') {
		...
		$value = str_replace ( '\\', '', $value );
		$form = '<input type="text" id="' . $keyval . '" name="' . $keyval . '" value="' . $value . '" ';
		if (is_int ( $style )) {
			$form .= 'size="' . $style . '" ';
		} else {
	...
	function qPassword($keyval, $default = '', $style = '', $xtra = '') {
		...
		$form = '<input type="password" id="' . $keyval . '" name="' . $keyval . '" ';
		if (is_int ( $style )) {
			$form .= 'size="' . $style . '" ';
		} else {
			$form .= Tzn::_style ( $style );
	...
	function qTextArea($keyval, $default = '', $style = '', $xtra = '') {
		if (is_object ( $this )) {
	...
	function qCheckbox($keyval, $default = 0, $style = '', $xtra = '') {
		$value = Tzn::getBol ( $keyval, $default );
		$form = '<input type="checkbox" id="' . $keyval . '" name="' . $keyval . '" value="1" ';
		$form .= Tzn::_style ( $style );
		if ($value) {
			$form .= 'checked="true" ';
		}
	...
	function qCheckbox2($name, $value, $checked = false, $style = '', $xtra = '') {
		$form = '<input type="checkbox" id="' . $name . '" name="' . $name . '" value="' . $value . '" ';
		$form .= Tzn::_style ( $style );
		if ($checked) {
			$form .= 'checked="true" ';
		}
	...
	function qSelect($name, $keyval, $default = '', $nochoice = '', $style = 'tznFormSelect', $xtra = '') {
	...
	function qSelect2($name, $key, $value, $default = '', $nochoice = '', $style = 'tznFormSelect', $xtra = '') {
		$form = '<select name="' . $name . '" ';
		$form .= 'id="' . $name . '" ';
		$form .= Tzn::_style ( $style );
		if ($xtra) {
	...
	function qDateTime($keyval, $tz = TZN_TZDEFAULT, $default = '', $style = 'tznFormDate', $xtra = '') {
		if (is_object ( $this )) {
		...
		$form = ', <input type="text" name="' . $keyval . 'Time" value="' . substr ( $value, 11, 5 ) . '" ';
		if (is_int ( $style )) {
			$form .= 'size="' . $style . '" ';
		} else {
			$form .= Tzn::_style ( $style );
	...
	function qSubmit($keyval, $default = '', $style = '', $xtra = '') {
		...
		$form = '<input type="submit" id="' . $keyval . '" name="' . $keyval . '" value="' . $value . '" ';
		if (is_int ( $style )) {
			$form .= 'size="' . $style . '" ';
		} else {
			$form .= Tzn::_style ( $style );
	...
	function qButton($keyval, $default = '', $style = '', $xtra = '') {
		...
		$form = '<input type="button" id="' . $keyval . '" name="' . $keyval . '" value="' . $value . '" ';
		if (is_int ( $style )) {
			$form .= 'size="' . $style . '" ';
		} else {
			$form .= Tzn::_style ( $style );
	...
	function qReset($keyval, $default = '', $style = '', $xtra = '') {
		if (is_object ( $this )) {
			$value = Tzn::get ( $keyval, $default );
		} else {
			$value = $default;
	...
	function checkEmpty($fields) {
		...
		foreach ( $arrFields as $field ) {
			$field = trim ( $field );
			if ((! $this->$field) || ((is_object ( $this->$field )) && (! $this->$field->id))) {
	...
	function qSession() {
		if (@constant ( 'TZN_TRANS_ID' )) {
			print '<input type="hidden" name="' . ini_get ( 'session.name' ) . '" value="' . session_id () . '" />';
		}
	}
	...
	function getHttpParameter($method, $key, $save = false, $recover = false) {
		...
		// error_log('PARAM '.$key.': ['.$method.']='.$arrValue[$key].' /
	...
	/*
	 * function redirect($url,$message='',$forceRef=false) { if
	 * (@constant('TZN_TRANS_ID')) { if (session_id() &&
	 * (!preg_match('/'.session_id().'/',$url))) { $url =
	 * Tzn::concatUrl($url,session_name() .'='.session_id()); } } if ($message)
	 * { $message = preg_replace("/<script[^>]*>[^<]+<\/script[^>]*>/is" ,"",
	 * $message); $message = preg_replace("/<\/?(div|span|iframe|frame|input|"
	 * ."textarea|script|style|applet|object|embed|form)[^>]*>/is" ,"",
	 * $message); if (@constant('TZN_TRANS_STATUS')) { $_SESSION['tznMessage'] =
	 * $message; } else { $url =
	 * Tzn::concatUrl($url,'tznMessage='.urlencode($message)); } } if
	 * ($forceRef) { $url =
	 * Tzn::concatUrl($url,'ref='.rawurlencode($_SERVER['REQUEST_URI'])); }
	 * header("Location: ".str_replace('&amp;','&',$url)); exit; }
	 */
	function redirect($url, $message = '', $mode = 0, $forceRef = false) {
		if (@constant ( 'TZN_TRANS_ID' )) {
		...
		if ($message) {
			...
			$message = preg_replace ( "/<\/?(div|span|iframe|frame|input|" . "textarea|script|style|applet|object|embed|form)[^>]*>/is", "", $message );
			$url = Tzn::concatUrl ( $url, 'msg=' . urlencode ( $message ) );
		}
		if ($mode) {
			$url = Tzn::concatUrl ( $url, 'm=' . $mode );
		}
		if ($forceRef) {
...
class TznCollection {
	...
	function qSelect($name, $default = null, $optional = false, $style = 'tznFormSelect', $xtra = '') {
		$form = '<select name="' . $name . '" ';
		$form .= Tzn::_style ( $style );
		if ($xtra) {
			$form .= $xtra;
